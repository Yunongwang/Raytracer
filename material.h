...
class metal : public material {
    public:
        metal(const vec3& a, float f) : albedo(a) {if (f < 1)
        {fuzz = f;} else {fuzz =1;}
        }
...
class blinn : public material {
    public:
        blinn(const vec3& _base, const vec3& _lightpositon, const vec3& _lightcolor){
            base = _base; lightpositon = unit_vector(_lightpositon); lightcolor =_lightcolor;
            shiness = 10.0; kSpecular = 0.8; kDiffuse =0.8;
        };
